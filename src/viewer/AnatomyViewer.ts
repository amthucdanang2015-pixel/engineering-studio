...
import type { PBRConfig, ViewPreset } from "../core/domain/vehicle";
...
export class AnatomyViewer {
  ...
  // ── Semantic material-name API ─────────────────────────────────────────────

  /**
   * Apply a PBRConfig to ALL meshes that share the given GLB material name.
   * This is the primary entry point for the customization adapter.
   * Example: updateMaterialByName("paint", { color: "#ff0000" }) tints
   * every mesh on the Porsche that uses the "paint" material.
   */
  updateMaterialByName(materialName: string, config: PBRConfig) {
    this.assets.updateMaterialByName(materialName, config);
    this.dirty = true;
  }

  /**
   * Returns the list of GLB material names discovered in the current asset.
   * The UI uses this to know which customization sections to show.
   */
  getMaterialInventory(): string[] {
    return this.assets.getMaterialInventory();
  }

  /**
   * Returns the VehicleCapabilities report generated by GlbAnalyzer for the
   * currently loaded asset. Returns null if no model is loaded.
   * This is the primary data source for the Build UI.
   */
  getCapabilities() {
    return this.assets.getCapabilities();
  }

  /**
   * Resets ALL materials to the original GLB appearance.
   * Does NOT reload the GLB file — operates purely on cached material state.
   */
  resetCustomization() {
    this.assets.resetAllToOriginal();
    this.dirty = true;
  }

  // ── Legacy per-mesh-name update (Part Inspector click flow) ─────────────────
